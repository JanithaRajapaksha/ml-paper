...
Performance was evaluated using MAE, MSE, RMSE, and $R^2$. For $n$ observations with actual values $y_i$ and predictions $\hat{y}_i$, these metrics are
\begin{align}
...
\end{align}
Here, $\bar{y}$ is the mean observed target. Lower error values are better, while $R^2$ closer to one indicates stronger performance relative to the mean baseline.
